...
<body>
	...
	scope: pageContext(페이지 객체), request(포워딩해도 요청 안에서 전달됨), session, application
	<c:set var="browser" value="${header['User-Agent']}" />
	<%-- 위의 한줄과 같은 기능을 함.  변환 코드: 
	pageContext.setAttribute("browser", request.getHeader("User-Agent"));
	--%>
	<br>
	<c:out value="${browser}" /> <!-- 변수 값을 HTML로 안전하게 출력(escape) 해주는 태그 , <script>태그를 문자 그대로 출력해줘서 XSS같은 공격에 안전함 -->
	<p>
		browser 변수 제거 후
		<!-- pageContext.removeAttribute랑 같은효과임 -->
	...
	${user.age}           <!-- user 객체의 age 속성, user.getAge()랑 같은코드임 -->
	...
	<!-- 4) 논리 연산 -->
	${age > 18}       <!-- true/false -->
	${gender == 'M'}  <!-- 비교, EL언어는 ''(작은따옴표)를 선호, 큰따옴표도 되긴하는데 큰따옴표가 중복되면 문자열이 끊겨서 그냥 작은따옴표 써라. -->
	${gender != 'M'}
	${gender eq 'M'}  <!-- 비교(가독성), 위와 같은 코드 -->
	${gender ne 'M'}  
	
	<!-- 5) 스코프 지정 
		EL은 기본적으로 page -> request -> session -> application 순서로 변수 탐색 , 우선순위가 page부터긴한데 애초에 개발할때 변수명을 달리하고 개발해야겠지. -->
	${pageScope.temp}
	${requestScope.name}
	${sessionScope.user}
