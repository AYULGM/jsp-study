...
<body>
	...
	<script type="text/javascript">
		function checkMember() {
			...
			// @: 반드시 하나
			// [] 안의 .이 아닌 그냥 .자체는 .x(임의의한문자를표현한다.)라는 의미가 있어서 이스케이프문자 \ 를 붙여서 \. 이라고 해야한다.
			// 도메인 부분도 동일한 패턴(@뒤에를 도메인이라 그럼)
			// \.[a-zA-Z]{2,3}: 점(.) + 알파벳 2~3글자
			// 예: .kr, .com, .net 등
			
			// (참고) 더 간단하면서 널리 쓰이는 이메일 검증 패턴 (인터넷에 정규표현식 검색하면 사람마다 다 다르게 표현함)
			...
			if (!regExpId.test(id)) { // 정규 표현식 객체의 test메소드
				alert("아이디는 문자로 시작해 주세요!");
				return;
			}
			if (!regExpName.test(name)) { // 정규 표현식 객체의 test메소드
				alert("이름은 한글만으로 2~4글자 입력해 주세요!");
				return;
			}
			if (!regExpPasswd.test(passwd)) { // 정규 표현식 객체의 test메소드
			...
			form.submit(); // type이 submit이 아니라 button이라 서버에 전송하려면 필요함
		}
	</script>
</body>
</html>